...
#include <private/ui/mb_clipper.h>
...
namespace lsp
{
    namespace plugui
    {
        ...
        template <class T>
        T *mb_clipper_ui::find_split_widget(const char *fmt, const char *base, size_t id)
        {
            char widget_id[64];
        ...
        mb_clipper_ui::mb_clipper_ui(const meta::plugin_t *meta): ui::Module(meta)
        {

        }

        mb_clipper_ui::~mb_clipper_ui()
        {

        }

        status_t mb_clipper_ui::slot_split_mouse_in(tk::Widget *sender, void *ptr, void *data)
        {
            // Fetch parameters
            mb_clipper_ui *ui = static_cast<mb_clipper_ui *>(ptr);
            if (ui == NULL)
        ...
        status_t mb_clipper_ui::slot_split_mouse_out(tk::Widget *sender, void *ptr, void *data)
        {
            // Fetch parameters
            mb_clipper_ui *ui = static_cast<mb_clipper_ui *>(ptr);
            if (ui == NULL)
        ...
        ui::IPort *mb_clipper_ui::find_port(const char *fmt, const char *base, size_t id)
        {
            char port_id[32];
        ...
        mb_clipper_ui::split_t *mb_clipper_ui::find_split_by_widget(tk::Widget *widget)
        {
        ...
        void mb_clipper_ui::on_split_mouse_in(split_t *s)
        {
            if (s->wNote != NULL)
            {
        ...
        void mb_clipper_ui::on_split_mouse_out()
        {
            for (size_t i=0, n=vSplits.size(); i<n; ++i)
            {
                split_t *d = vSplits.uget(i);
        ...
        void mb_clipper_ui::add_splits()
        {
            static const char *fmt = "%s_%d";

            for (size_t port_id=1; port_id<meta::mb_clipper::BANDS_MAX; ++port_id)
            {
                split_t s;
                ...
                s.pFreq         = find_port(fmt, "xf", port_id);
        ...
        void mb_clipper_ui::update_split_note_text(split_t *s)
        {
            // Get the frequency
            ...
            // Update the note name displayed in the text
            {
                ...
                if (note_full != dspu::NOTE_OUT_OF_RANGE)
                {
                    ...
                    s->wNote->text()->set("lists.mb_clipper.notes.full", &params);
                }
                else
                    s->wNote->text()->set("lists.mb_clipper.notes.unknown", &params);
            }

        }

        status_t mb_clipper_ui::post_init()
        {
        ...
        void mb_clipper_ui::notify(ui::IPort *port, size_t flags)
        {
            for (size_t i=0, n=vSplits.size(); i<n; ++i)
            {
                split_t *d = vSplits.uget(i);
